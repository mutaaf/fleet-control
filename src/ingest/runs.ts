...
export function ingestProjectRuns(db: DB, cfg: FleetConfig, projectId: number, slug: string): number {
  ...
  const up = db.prepare(`
    ...
    ON CONFLICT(project_id,phase,session_id) DO UPDATE SET
      ...
      usage_limit_at=COALESCE(excluded.usage_limit_at, usage_limit_at),
      usage_limit_until=COALESCE(excluded.usage_limit_until, usage_limit_until)`);
  ...
  for (const line of complete.split("\n")) {
    ...
    const u = r.usage ?? {};
    const usage = detectUsageLimit(r.result_head ?? "");
    const ulAt = usage.hit ? (r.ts_end ?? r.ts_start ?? null) : null;
